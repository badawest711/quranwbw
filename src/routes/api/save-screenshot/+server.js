...
import { ARABIC_BOT_TOKEN, ARABIC_CHAT_ID, SAVE_LOCAL, ARABIC_SCREENSHOTS_DIR } from '$env/static/private';
...
export async function POST({ request }) {
	try {
		...
		if (SAVE_LOCAL === 'true') {
			await mkdir(ARABIC_SCREENSHOTS_DIR, { recursive: true });
			await writeFile(join(ARABIC_SCREENSHOTS_DIR, filename), buffer);
			for (const { name, base64: ab64, format = 'wav' } of audiosBase64) {
				...
				await writeFile(join(ARABIC_SCREENSHOTS_DIR, audioFilename), audioBuf);
			}
		}

		if (PUBLIC_TELEGRAM_ENABLED === 'true') {
			const formData = new FormData();
			formData.append('chat_id', ARABIC_CHAT_ID);
			...
			const res = await fetch(`https://api.telegram.org/bot${ARABIC_BOT_TOKEN}/sendPhoto`, {
				method: 'POST',
				body: formData
			});
			...
			if (audiosBase64.length > 0) {
				...
				for (const { name, base64: ab64, format = 'wav' } of audiosBase64) {
					...
					audioFormData.append('chat_id', ARABIC_CHAT_ID);
					audioFormData.append('audio', new Blob([audioBuf], { type: mimeType }), audioFilename);
					audioFormData.append('performer', name);
					const audioRes = await fetch(`https://api.telegram.org/bot${ARABIC_BOT_TOKEN}/sendAudio`, {
						method: 'POST',
						body: audioFormData
					});
				...
				const audioRes = await fetch(`https://api.telegram.org/bot${ARABIC_BOT_TOKEN}/sendAudio`, {
					...
					body: JSON.stringify({ chat_id: ARABIC_CHAT_ID, audio: audioUrls[0] })
				});
				const audioResult = await audioRes.json();
